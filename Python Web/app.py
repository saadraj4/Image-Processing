...
def upload():
    ...
    if filter_type == 'gaussian':
        ...
        template_filename = save_image(template_image, app.config['UPLOAD_FOLDER'],
                                       'template_' + template_file.filename)

        filtered_image = histogram_matching(image, template_image)
        filtered_filename = save_image(filtered_image, app.config['RESULT_FOLDER'], 'filtered_' + file.filename)

        return render_template('result.html', original_image=original_filename, reference_image=template_filename,
